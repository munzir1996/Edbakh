        ...
        <section class="row mt-3 justify-content-between">
            ...
            <recipe_ingredients inline-template>
                ...
                <div class="col-md-8 recipe_ingredients_section_tow">
                    ...
                    <div class="recipe_amount_number">
                        ...
                        <button @click="changeServe(2)" :class="{'recipe_amount_btn_active': this.serve == true}"
                            class="recipe_amount_btn {{App::isLocale('en')? 'border_top_right_zero border-bottom_right_zero': 'border_top_left_zero border_bottom_left_zero'}}">2</button>

                        <button @click="changeServe(4)" :class="{'recipe_amount_btn_active': this.serve == false}"
                            class="recipe_amount_btn {{App::isLocale('en')? 'border_top_left_zero border_bottom_left_zero': 'border_top_right_zero border-bottom_right_zero'}}">4</button>
                    ...
                    <div class="col-12 row">
                        ...
                        <div class="recipe_an_ingredient_container col-12 col-sm-6">
                            ...
                            <div class="recipe_an_ingredient_text_container">
                                ...
                                <p v-if="this.serve">
                                    {{$contain->pivot->amount}} {{$contain['weight_'.App::getLocale()]}}
                                </p>
                                <p v-else>
                                    {{2 * $contain->pivot->amount}} {{$contain['weight_'.App::getLocale()]}}
                                </p>
                    ...
                    <div class="col-12 row">
                        <h4 style="margin-top: 50px">
                            <span>
                                {{__('anrecipe.not_included')}}
                            </span>
                        </h4>
                    </div>
                    <div class="col-12 row">
                        @foreach ($recipe->contains as $contain)
                        @if ($contain->pivot->include === 0)
                        <div class="recipe_an_ingredient_container col-12 col-sm-6">
            ...
            <div class="col-md-4 recipe_natural_section_tow row pb-5">
                ...
                <div class="justify-content-between col-12 mb-4">
                    ...
                    <span
                        class="nutrition_value_title {{App::isLocale('en')? 'float-right': 'float-left'}}">{{__('anrecipe.per_serving')}}</span>
                </div>

                <div class="justify-content-between col-12 mb-3">
                    <span class="nutrition_name">{{__('anrecipe.nutrition1')}}</span>
                    <span
                        class="nutrition_value {{App::isLocale('en')? 'float-right': 'float-left'}}">{{__('anrecipe.nutrition1_value')}}</span>
                </div>

                <div class="justify-content-between col-12 mb-3">
                    <span class="nutrition_name">{{__('anrecipe.nutrition2')}}</span>
                    <span
                        class="nutrition_value {{App::isLocale('en')? 'float-right': 'float-left'}}">{{$recipe->calories}}
                        {{__('anrecipe.nutrition2_value')}}</span>
                </div>
                ...
                <div class="justify-content-between col-12 mb-3">
                    ...
                    <span
                        class="nutrition_value {{App::isLocale('en')? 'float-right': 'float-left'}}">{{$nutrition->fats}}
                        {{__('anrecipe.nutrition3_value')}}</span>
                </div>

                <div class="justify-content-between col-12 mb-3">
                    <span class="nutrition_name">{{__('anrecipe.nutrition4')}}</span>
                    <span
                        class="nutrition_value {{App::isLocale('en')? 'float-right': 'float-left'}}">{{$nutrition->saturated_fats}}
                        {{__('anrecipe.nutrition4_value')}}</span>
                </div>

                <div class="justify-content-between col-12 mb-3">
                    <span class="nutrition_name">{{__('anrecipe.nutrition5')}}</span>
                    <span
                        class="nutrition_value {{App::isLocale('en')? 'float-right': 'float-left'}}">{{$nutrition->carbohydrate}}
                        {{__('anrecipe.nutrition5_value')}}</span>
                </div>

                <div class="justify-content-between col-12 mb-3">
                    <span class="nutrition_name">{{__('anrecipe.nutrition6')}}</span>
                    <span
                        class="nutrition_value {{App::isLocale('en')? 'float-right': 'float-left'}}">{{$nutrition->sugar}}
                        {{__('anrecipe.nutrition6_value')}}</span>
                </div>

                <div class="justify-content-between col-12 mb-3">
                    <span class="nutrition_name">{{__('anrecipe.nutrition7')}}</span>
                    <span
                        class="nutrition_value {{App::isLocale('en')? 'float-right': 'float-left'}}">{{$nutrition->dietary_fiber}}
                        {{__('anrecipe.nutrition7_value')}}</span>
                </div>

                <div class="justify-content-between col-12 mb-3">
                    <span class="nutrition_name">{{__('anrecipe.nutrition8')}}</span>
                    <span
                        class="nutrition_value {{App::isLocale('en')? 'float-right': 'float-left'}}">{{$nutrition->protein}}
                        {{__('anrecipe.nutrition8_value')}}<span>
                </div>

                <div class="justify-content-between col-12 mb-3">
                    <span class="nutrition_name">{{__('anrecipe.nutrition9')}}</span>
                    <span
                        class="nutrition_value {{App::isLocale('en')? 'float-right': 'float-left'}}">{{$nutrition->cholestrol}}
                        {{__('anrecipe.nutrition9_value')}}</span>
                </div>

                <div class="justify-content-between col-12">
                    <span class="nutrition_name">{{__('anrecipe.nutrition10')}}</span>
                    <span
                        class="nutrition_value {{App::isLocale('en')? 'float-right': 'float-left'}}">{{$nutrition->sodium}}
                        {{__('anrecipe.nutrition10_value')}}</span>
                </div>
                @endforeach
            </div>
                ...
                <div class="col-md-7 instruction_description_container">
                    ...
                    <div class="instruction_description" style="border: none !important;">
                        ...
                        <div style="display: inline-block">
                            <div class="instruction_number">{{$instruction->step}}</div>
                            <span class="instruction_step_title">{{$instruction['title_'.App::getLocale()]}}:</span>
                        </div>
